...
//	}
	...
	protected int receivePacket() {
		...
		if (m_cs) {
			...
			res = receive(buffer, length, csCalc);
//			res = link.read(buffer, length);
//			for (int i = 0; i < length-1; i++) {
//				int csb = buffer[i] & 0xff;
//				csCalc.updateChecksum(csb);
//			}
	...
	protected int sendPacket() {
		...
		// Send buffer
		return link.write(buffer, (byte) (length + PIXY_SEND_HEADER_SIZE));
//		return link.send();
	}
